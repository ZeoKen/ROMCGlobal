...
function FunctionGuild:SetGuildFlagIcon(flag_guid, city_id)
  local city_show_data = GvgProxy.Instance:GetGvGCityShowData(city_id)
  if not city_show_data then
    return
  end
  self:SetGuildLandIcon(flag_guid, city_show_data.portrait, city_show_data.guildid)
end
